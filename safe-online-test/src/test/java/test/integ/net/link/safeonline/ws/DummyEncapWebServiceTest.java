...
public class DummyEncapWebServiceTest {
    ...
    public void testEncapChallenge() throws MalformedURLException, RemoteException {
        ...
        URL endpointURL = new URL(endpoint);
        MSecBankIdSoapBindingStub authStub = new MSecBankIdSoapBindingStub(endpointURL, new Service());

        // operate
        ChallengeResponse challengeResponse = authStub.challenge(VALID_MOBILE, "-1");
        // verify
        assertEquals(EncapConstants.ENCAP_SUCCES, challengeResponse.getStatus());
        // operate
        VerifyResponse verifyResponse = authStub.verifyOTP(challengeResponse.getChallengeId(), "000000");
        // verify
