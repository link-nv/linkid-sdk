...
public class PagingTest {
    ...
    public void testPaging()
        ...
        DriverProfileEntity profile = (DriverProfileEntity) em.createQuery(
                "SELECT p FROM DriverProfileEntity p" + "    JOIN p.execution e" + "    WHERE e.startTime = :startTime"
                        + "        AND p.driverName = :driverName").setParameter("startTime", startTime).setParameter("driverName",
                driverName).getSingleResult();
    ...
    public Set<ProfileDataEntity> testPagingOne(DriverProfileEntity profile, int dataPoints)
        ...
        Query profileDataQuery = em.createQuery(
                "SELECT d FROM ProfileDataEntity d" + "    WHERE d.profile = :profile" + "    ORDER BY d.scenarioStart").setParameter(
        ...
        for (int point = 0; (profileData = profileDataQuery.setFirstResult(point * period).getResultList()) != null; ++point) {
            ...
            ProfileDataEntity data = new ProfileDataEntity(profileData.get(0).getProfile(), profileData.get(0).getScenarioTiming());
            pointData.add(data);

            Set<MeasurementEntity> measurements = new HashSet<MeasurementEntity>();
            for (String measurement : durations.keySet()) {
                measurements.add(new MeasurementEntity(data, measurement, durations.get(measurement) / counts.get(measurement)));
            }
    ...
    public Set<ProfileDataEntity> testPagingTwo(DriverProfileEntity profile, int dataPoints)
        ...
        long dataDuration = (Long) em.createQuery(
                "SELECT MAX(d.scenarioStart) - MIN(d.scenarioStart)" + "    FROM ProfileDataEntity d            "
                        + "    WHERE d.profile = :profile          ").setParameter("profile", profile).getSingleResult();
        ...
        System.err.println("period = dataDuration (" + dataDuration + ") / dataPoints (" + dataPoints + ") = " + period);
        ...
        for (long point = 0; point * period < dataDuration; ++point) {
            ...
            List<MeasurementEntity> measurements = em.createQuery(
                    "SELECT NEW net.link.safeonline.performance.entity.MeasurementEntity(" + "        m.measurement, AVG(m.duration)"
                            + "    )                                 " + "    FROM ProfileDataEntity d          "
                            + "        JOIN d.measurements m         " + "    WHERE d.profile = :profile        "
                            + "    AND d.scenarioStart > :start      " + "    AND d.scenarioStart <= :stop      "
                            + "    GROUP BY m.measurement            ").setParameter("profile", profile).setParameter("start",
