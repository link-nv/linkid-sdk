    ...
    public PlayTest() throws Exception {
        ...
        ttt = em.createQuery(
                "SELECT d.scenarioTiming.startTime, d.scenarioTiming.agentDuration, d.scenarioTiming.olasDuration, m.measurement, m.duration"
                        ...
                        + "        AND d.scenarioTiming.startTime < :stop      " + "        AND m.measurement = :measurement")
                     .setParameter("profile", profile).setParameter("start", dataStart + point * period).setParameter("stop",
                             dataStart + (point + 1) * period).setParameter("measurement", ProfileData.REQUEST_DELTA_TIME).getResultList();
