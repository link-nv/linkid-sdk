		...
		try {
			...
			try {
				ResultSet resultSet = statement
						.executeQuery("SELECT value FROM metadata WHERE name='"
				...
				try {
					if (resultSet.next()) {
						...
						if (false == DatabaseConstants.VERSION.equals(value)) {
							System.out
									.println("Database schema version should be: "
											+ DatabaseConstants.VERSION);
						} else {
							System.out
									.println("Database schema version is OK.");
						}
					} else {
						System.out
								.println("Database schema has no version in metadata table.");
						System.out
								.println("Database has been incorrectly initialized.");
					}
				} finally {
					resultSet.close();
				}
