...
public class DatabasePluginManager {
	...
	public static Connection connect(DatabasePlugin databasePlugin,
			String connectionUrl, String user, String password)
			throws SQLException {
		if (null != activeConnection) {
			throw new IllegalStateException("already an active connection");
		}
		Connection connection = databasePlugin.getConnection(connectionUrl,
				user, password);
	...
	public static void disconnect() {
		...
		try {
			...
			throw new RuntimeException(
					"error disconnecting: " + e.getMessage(), e);
		}
