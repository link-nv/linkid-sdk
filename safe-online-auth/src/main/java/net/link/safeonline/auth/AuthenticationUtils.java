...
public class AuthenticationUtils {
	...
	/**
	 * Redirects the authentication webapp to the external device landing page
	 * using the SAML browser post protocol. The SAML authentication request is
	 * generated by the authentication service associated with this HTPP
	 * session.
	 * <p>
	 * The method requires the <code>DeviceAuthenticationServiceUrl</code>
	 * init parameter defined in web.xml pointing to the location of the
	...
	public static String redirectAuthentication(String landingUrl, String device) {
		...
		AuthenticationService authenticationService = AuthenticationServiceManager
				.getAuthenticationService(httpRequest.getSession());

		String encodedSamlRequestToken;
		try {
			encodedSamlRequestToken = authenticationService
					.redirectAuthentication(authenticationServiceUrl,
							encodedLandingUrl, device);
		} catch (NodeNotFoundException e) {
			throw new RuntimeException(
					"could not initiate authentication: Node not found :"
							+ e.getMessage(), e);
		}
	...
	/**
	 * Redirects the authentication to the external device registration landing
	 * page using the SAML browser post protocol. The SAML authentication
	 * request is generated by the authentication service associated with this
	 * HTPP session.
	 * <p>
	 * The method requires the <code>DeviceRegistrationServiceUrl</code> init
	 * parameter defined in web.xml pointing to the location of the
	 * authentication web application location the device issuer should return
	 * to.
	 * </p>
	 * <p>
	...
	public static String redirect(String landingUrl, String device,
		...
		String registrationServiceUrl = getInitParameter(externalContext,
				DEVICE_REG_SERVICE_URL_INIT_PARAM);
		...
		try {
			encodedSamlRequestToken = authenticationService
					.redirectRegistration(registrationServiceUrl,
							encodedLandingUrl, device, username);
		} catch (SafeOnlineException e) {
			throw new RuntimeException(
					"could not initiate device registration:" + e.getMessage(),
					e);
		}
