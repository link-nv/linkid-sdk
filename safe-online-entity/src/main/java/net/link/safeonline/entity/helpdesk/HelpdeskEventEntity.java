...
@NamedQueries( {
        @NamedQuery(name = QUERY_WHERE_CONTEXTID, query = "SELECT event " + "FROM HelpdeskEventEntity AS event "
                + "WHERE event.helpdeskContext.id = :contextId"),
        @NamedQuery(name = QUERY_DELETE_WHERE_CONTEXTID, query = "DELETE " + "FROM HelpdeskEventEntity AS event "
                + "WHERE event.helpdeskContext.id = :contextId"),
        @NamedQuery(name = QUERY_DELETE_WHERE_OLDER, query = "DELETE " + "FROM HelpdeskEventEntity AS event "
...
public class HelpdeskEventEntity implements Serializable {
    ...
    public HelpdeskEventEntity(String message, String principal, LogLevelType logLevel) {
    ...
    public interface QueryInterface {
        ...
        List<HelpdeskEventEntity> listLogs(@QueryParam("contextId") Long contextId);
        ...
        void deleteEvents(@QueryParam("ageLimit") Date ageLimit, @QueryParam("logLevel") LogLevelType logLevel);

        @QueryMethod(QUERY_LIST_USER_CONTEXTS)
        List<HelpdeskContextEntity> listUserContexts(@QueryParam("principal") String user);
