...
public class Pcsc {
	 ...
	 */
	public static final byte[] EF_ID_RN = new byte[] { 0x3F, 0x00, (byte) 0xDF,
			0x01, 0x40, 0x31 };

	/**
	 * 3F00 = MF, DF01 = DF(ID), 4032 = EF(SGN#RN)
	 */
	public static final byte[] EF_SGN_RN = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x01, 0x40, 0x32 };

	/**
	 * 3F00 = MF, DF01 = DF(ID), 4033 = EF(ID#Address)
	 */
	public static final byte[] EF_ID_ADDRESS = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x01, 0x40, 0x33 };

	/**
	 * 3F00 = MF, DF01 = DF(ID), 4034 = EF(SGN#Address)
	 */
	public static final byte[] EF_SGN_ADDRESS = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x01, 0x40, 0x34 };

	/**
	 * 3F00 = MF, DF01 = DF(ID), 4035 = EF(ID#Photo)
	 */
	public static final byte[] EF_ID_PHOTO = new byte[] { 0x3F, 0x00,
	 ...
	 */
	public static final byte[] EF_CERT_2 = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x00, 0x50, 0x38 };

	/**
	 * 3F00 = MF, DF00 = DF(BELPIC), 5039 = EF(Cert#3)
	 */
	public static final byte[] EF_CERT_3 = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x00, 0x50, 0x39 };

	/**
	 * 3F00 = MF, DF00 = DF(BELPIC), 503A = EF(Cert#4)
	 */
	public static final byte[] EF_CERT_4 = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x00, 0x50, 0x3A };

	/**
	 * 3F00 = MF, DF00 = DF(BELPIC), 503B = EF(Cert#6)
	 */
	public static final byte[] EF_CERT_6 = new byte[] { 0x3F, 0x00,
			(byte) 0xDF, 0x00, 0x50, 0x3B };

	/**
	 * 3F00 = MF, DF00 = DF(BELPIC), 503C = EF(Cert#8)
	 */
	public static final byte[] EF_CERT_8 = new byte[] { 0x3F, 0x00,
	...
	public IdentityFile getIdentityFile() throws CardException {
		...
		IdentityFile identifyFile = parseTagLengthValueData(data,
				IdentityFile.class);
		return identifyFile;
	}
	...
	public AddressFile getAddressFile() throws CardException {
		...
		AddressFile addressFile = parseTagLengthValueData(data,
				AddressFile.class);
		return addressFile;
	}
	...
	public boolean verifyRnSignature() throws CardException,
	...
	public boolean verifyAddressSignature() throws CardException,
			CertificateException, InvalidKeyException, SignatureException {
	...
	public byte[] sign(byte[] data, String pin) throws CardException,
			IOException {
		MessageDigest hash;
		try {
			hash = MessageDigest.getInstance("SHA1");
		...
		CommandAPDU setApdu = new CommandAPDU(
				0x00,
				0x22,
				0x41,
				0xB6,
				new byte[] { 0x04, (byte) 0x80, 0x01, (byte) 0x84, (byte) 0x82 });
		...
		byte[] ALGORITHM_IDENTIFIER_SHA1 = new byte[] { 0x30, 0x21, 0x30, 0x09,
	...
	private void checkResponseSW(ResponseAPDU responseApdu) {
		if (0x90 != responseApdu.getSW1()) {
			...
			throw new RuntimeException("SW1: "
					+ Integer.toHexString(responseApdu.getSW1()));
		}
		if (0x00 != responseApdu.getSW2()) {
			LOG.debug("SW1-SW2: " + Integer.toHexString(responseApdu.getSW1())
					+ "-" + Integer.toHexString(responseApdu.getSW2()));
			throw new RuntimeException("SW2: "
					+ Integer.toHexString(responseApdu.getSW2()));
		}
	}
	...
	private <T> T parseTagLengthValueData(byte[] data, Class<T> type) {
		...
		try {
			...
			throw new RuntimeException("illegal access error on: "
					+ type.getName());
		}
	...
	private void setValue(Field field, byte[] value, Object object) {
		...
		try {
			...
			if (null != convertAnnotation) {
				Class<? extends Convertor<?>> convertorClass = convertAnnotation
						.value();
				...
				try {
					...
					throw new RuntimeException(
							"could not instantiate convertor: "
									+ convertorClass.getName());
				}
				try {
			...
			throw new RuntimeException("field type incorrect: "
