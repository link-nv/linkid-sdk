...
public class GetEncapAuthenticationPortImpl implements GetDeviceAuthenticationPort {

    private static final Log LOG = LogFactory.getLog(GetEncapAuthenticationPortImpl.class);


    @WebMethod
    public synchronized AuthenticationGetInstanceResponseType getInstance(AuthenticationGetInstanceRequestType request) {

        LOG.debug("return instance of stateful authentication webservice for encap device");
        AuthenticationGetInstanceResponseType response = new AuthenticationGetInstanceResponseType();
        response.setEndpoint(EncapAuthenticationPortImpl.manager.export(new EncapAuthenticationPortImpl()));
        return response;
