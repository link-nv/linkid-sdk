...
public class StatisticServiceBean implements StatisticService,
	...
	public JFreeChart getChart(String statisticName, String statisticDomain,
		...
		StatisticEntity statistic = this.getStatistic(statisticName,
				statisticDomain, applicationName);
	...
	public HSSFWorkbook exportStatistics(String applicationName)
		...
		List<StatisticEntity> statistics = this.statisticDAO
	...
	private void exportStatistic(StatisticEntity statistic,
		...
		row1.createCell((short) 0).setCellValue(
				new HSSFRichTextString("Statistic"));
		row1.createCell((short) 1).setCellValue(
				new HSSFRichTextString(statistic.getName()));
		HSSFRow row2 = mainSheet.createRow(1);
		row2.createCell((short) 0).setCellValue(
				new HSSFRichTextString("Application"));
		row2.createCell((short) 1).setCellValue(
				new HSSFRichTextString(statistic.getApplication().getName()));
		HSSFRow row3 = mainSheet.createRow(2);
		row3.createCell((short) 0).setCellValue(
				new HSSFRichTextString("Creation time"));
		...
		if (statistic.getName().equals(usageStatistic)) {
			row5.createCell((short) 11).setCellValue(
					new HSSFRichTextString("Number of Users"));
			row5.createCell((short) 12).setCellValue(
					new HSSFRichTextString("Number of Active Users"));
			row5.createCell((short) 13).setCellValue(
					new HSSFRichTextString("Number of Logins"));
		} else {
			row5.createCell((short) 11).setCellValue(
					new HSSFRichTextString("X"));
			row5.createCell((short) 12).setCellValue(
					new HSSFRichTextString("Y"));
			row5.createCell((short) 13).setCellValue(
					new HSSFRichTextString("Z"));
		...
		JFreeChart chart = this.getChart(statistic.getName(), statistic
				.getDomain(), statistic.getApplication().getName());

		byte[] image = null;

		try {
			image = ChartUtilities.encodeAsPNG(chart.createBufferedImage(600,
					800));
		} catch (Exception e) {
		...
		int pictureIndex = workbook.addPicture(image,
				HSSFWorkbook.PICTURE_TYPE_PNG);

		HSSFPatriarch patriarch = mainSheet.createDrawingPatriarch();
		HSSFClientAnchor anchor = new HSSFClientAnchor(0, 0, 0, 255, (short) 0,
				4, (short) 10, 34);
