...
public class MessageHandlerManager {
	...
	private static final Log LOG = LogFactory
			.getLog(MessageHandlerManager.class);
	...
	static {
		registerProtocolHandler(SafeOnlineConstants.TOPIC_REMOVE_USER,
				RemoveUserMessageHandler.class);
	}

	private static void registerProtocolHandler(String topic,
		...
		try {
			...
			throw new RuntimeException(
					"could not initialize protocol handler: "
							+ messageHandlerClass.getName() + "; message: "
							+ e.getMessage(), e);
		}
	}
	...
	public static void sendMessage(String topic, List<String> message,
		...
		NotificationConsumerClient consumerClient = new NotificationConsumerClientImpl(
				consumer.getAddress(), authIdentityServiceClient
		...
		if (null != consumer.getApplication()) {
			...
			returnMessage = messageHandler.createApplicationMessage(message,
			...
			returnMessage = messageHandler.createDeviceMessage(message,
					consumer.getDevice());
		}
	...
	public static void handleMessage(String topic, String destination,
			List<String> message) throws MessageHandlerNotFoundException {
		MessageHandler messageHandler = messageHandlerMap.get(topic);
		if (null == messageHandler)
			throw new MessageHandlerNotFoundException(topic);
		messageHandler.init();
