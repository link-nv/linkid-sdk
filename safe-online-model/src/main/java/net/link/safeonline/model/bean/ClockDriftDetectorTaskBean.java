...
/**
 * Clock Drift Detector Task. Detects local clock drift using an NTP server.
 * Since WS-Security and SAML depends on timestamps to prevent replay attacks
 * it's important to have a proper clock drift detection mechanism.
 * 
 * @author fcorneli
 * 
 */
...
public class ClockDriftDetectorTaskBean implements Task {
	...
	private static final Log LOG = LogFactory
			.getLog(ClockDriftDetectorTaskBean.class);
		...
		try {
			...
			if (Math.abs(offset) > this.maxClockOffset) {
				...
				this.securityAuditLogger.addSecurityAudit(
						SecurityThreatType.DISRUPTION,
						"Maximum clock offset reached of " + offset
								+ " milliseconds against NTP Server: "
								+ ntpServerAddress);
			}
		} catch (SocketException e) {
			this.resourceAuditLogger.addResourceAudit(ResourceNameType.NTP,
					ResourceLevelType.RESOURCE_UNAVAILABLE, this.ntpServerName,
