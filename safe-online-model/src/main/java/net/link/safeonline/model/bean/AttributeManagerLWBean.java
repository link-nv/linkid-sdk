...
/**
 * Lightweight attribute manager bean. The attribute manager is responsible for
 * lifecycle management of attributes.
 * 
 * <p>
 * This bean is not an EJB3 session bean, but a regular POJO that can be used
 * within the tx/security context of caller session beans. This is an
 * interesting pattern to keep the overhead of tx/security interceptors as low
 * as possible. Applying this pattern all over the place will certainly result
 * in an increase of performance. For lightweight beans we're using the LWBean
 * suffix.
 * </p>
 * 
 * @author fcorneli
...
public class AttributeManagerLWBean {
	...
	private static final Log LOG = LogFactory
			.getLog(AttributeManagerLWBean.class);
	 ...
	 * 
	 * <p>
	 * At this point the RBAC and owner access control checks should already
	 * have been performed.
	 * </p>
	 * 
	 * @param attributeType
	 ...
	 */
	public void removeAttribute(AttributeTypeEntity attributeType,
		...
		if (attributeType.isCompounded()) {
			throw new EJBException(
					"cannot remove compounded attributes via this method");
		}

		/*
		 * Else we're dealing with multi-valued attributes. In this case we're
		 * removing all of the multi-valued entries for the given attribute
		 * type.
		 */
	 ...
	 * 
	 * <p>
	 * In case of a multi-valued (compounded) attribute, only the attribute
	 * entry corresponding with the given attribute index will be removed.
	 * </p>
	 * 
	 * <p>
	 * At this point the RBAC and owner access control checks should already
	 ...
	 *            the index of the attribute entry to be removed in case of a
	 *            multi-valued (compounded) attribute.
	 * @param subject
	 ...
	 */
	public void removeAttribute(AttributeTypeEntity attributeType,
			long attributeIndex, SubjectEntity subject)
			throws PermissionDeniedException, AttributeNotFoundException,
			AttributeTypeNotFoundException {
		LOG.debug("remove attribute " + attributeType.getName()
	...
	/**
	 * Removes a compounded attribute record of the given attribute type for the
	 * given subject.
	 * 
	 * <p>
	 ...
	 * @throws AttributeNotFoundException
	 */
	public void removeCompoundAttribute(AttributeTypeEntity attributeType,
			SubjectEntity subject, String attributeId)
			throws AttributeNotFoundException {
		AttributeEntity compoundAttribute = getCompoundAttribute(attributeType,
				subject, attributeId);
		...
		for (CompoundedAttributeTypeMemberEntity member : members) {
			AttributeTypeEntity memberAttributeType = member.getMember();
			AttributeEntity memberAttribute = this.attributeDAO.findAttribute(
					subject, memberAttributeType, attributeIdx);
			if (null != memberAttribute) {
				/*
				 * It's allowed that some (i.e. the optional) member attribute
				 * entries are missing.
				 */
	...
	private AttributeEntity getCompoundAttribute(
