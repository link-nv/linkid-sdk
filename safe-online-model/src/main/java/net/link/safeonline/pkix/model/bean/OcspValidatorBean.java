...
public class OcspValidatorBean implements OcspValidator {
    ...
    public OcspResult verifyOcspStatus(URI ocspUri, X509Certificate certificate, X509Certificate issuerCertificate) {
        ...
        try {
            ...
            throw new RuntimeException("OCSP response IO error: " + e.getMessage(), e);
        }
        ...
        LOG.debug("OCSP response produced at: " + basicOCSPResp.getProducedAt());
        /*
         * Do not perform any check on productedAt since RFC 2560 - 2.5 allows for Response Pre-production.
         */
        ...
        try {
            ...
            throw new RuntimeException("no such provider error: " + e.getMessage(), e);
        } catch (OCSPException e) {
            throw new RuntimeException("OCSP error: " + e.getMessage(), e);
        }
        if (null == certs) {
        ...
        /*
         * It's possible that the OCSP Responder gives us the entire certificate chain.
         */
        ...
        try {
            if (false == issuerCertificate.equals(ocspResponderCertificate)) {
                ocspResponderCertificate.verify(issuerCertificate.getPublicKey());
            }
    ...
    private byte[] getOcsp(URI ocspUri, X509Certificate certificate, X509Certificate issuerCertificate) {
        ...
        try {
            ...
            throw new RuntimeException("OCSP URL connection error: " + e.getMessage(), e);
        }
        ...
        try {
            certId = new CertificateID(CertificateID.HASH_SHA1, issuerCertificate, certificate.getSerialNumber());
        } catch (OCSPException e) {
            throw new RuntimeException("OCSP error: " + e.getMessage(), e);
        }
        ...
        connection.setRequestProperty("Content-Length", Integer.toString(ocspData.length));
        ...
        LOG.debug("response OK: " + (HttpURLConnection.HTTP_OK == responseCode));
        ...
        ByteArrayOutputStream arrayOutputStream = new ByteArrayOutputStream(resultContentLength);
        try {
