...
import static net.link.safeonline.beid.webapp.BeIdMountPoints.MountPoint.REGISTRATION;
...
public class RegisterServlet extends HttpServlet {
    ...
    private void prepareJavaVersionCheck(HttpServletRequest req, HttpServletResponse resp)
        ...
        JavaVersionServlet.setPkcs11Target(REGISTRATION.linkFor(PKCS11), session);
        JavaVersionServlet.setJava15NoPkcs11Target(ERROR.linkFor(NO_MIDDLEWARE), session);
        JavaVersionServlet.setJava16NoPkcs11Target(REGISTRATION.linkFor(PCSC), session);
