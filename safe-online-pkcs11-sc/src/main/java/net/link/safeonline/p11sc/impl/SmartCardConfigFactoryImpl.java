...
public class SmartCardConfigFactoryImpl implements SmartCardConfigFactory {
	...
	public List<SmartCardConfig> getSmartCardConfigs() {
		...
		while (configResources.hasMoreElements()) {
			...
			String cardAlias = (String) properties
					.get("safe.online.pkcs11.sc.config.alias");
			SmartCardConfigImpl smartCardConfig = new SmartCardConfigImpl(
					cardAlias);
			...
			String signatureKeyAlias = (String) properties
					.get("safe.online.pkcs11.sc.config.keymapping.signature");
			...
			for (Object key : keys) {
				...
				if (false == keyStr
						.matches("safe\\.online\\.pkcs11\\.sc\\.config\\.library\\..*\\.match")) {
					continue;
				}
				LOG.debug("keyStr: " + keyStr);
				String iterId = keyStr
						.substring("safe.online.pkcs11.sc.config.library."
								.length());
				iterId = iterId.substring(0, iterId.indexOf("."));
				LOG.debug("iterId: " + iterId);

				String match = properties
						.getProperty("safe.online.pkcs11.sc.config.library."
								+ iterId + ".match");
				LOG.debug("match: " + match);
				List<String> driverLocations = new LinkedList<String>();
				for (Object locationKey : keys) {
					String locationKeyStr = (String) locationKey;
					if (false == locationKeyStr
							.matches("safe\\.online\\.pkcs11\\.sc\\.config\\.library\\."
									+ iterId + "\\.location\\..*")) {
						continue;
					}
				...
				smartCardConfig
						.addPkcs11DriverLocations(match, driverLocations);
			}
