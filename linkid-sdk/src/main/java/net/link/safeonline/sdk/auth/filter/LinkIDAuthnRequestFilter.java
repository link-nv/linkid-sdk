...
import net.link.safeonline.sdk.configuration.LinkIDSDKConfig;
 ...
 * authentication response is done via the {@link LinkIDAuthnResponseFilter}.
 * <p/>
 ...
 *
 * @author fcorneli
 * @see LinkIDAuthnResponseFilter
 */
public class LinkIDAuthnRequestFilter implements Filter {

    private static final Logger logger = Logger.get( LinkIDAuthnRequestFilter.class );
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (LinkIDLoginManager.isAuthenticated( httpRequest.getSession() ))
            chain.doFilter( httpRequest, httpResponse );
        else
            LinkIDAuthenticationUtils.login( httpRequest, httpResponse, newContext( httpRequest ) );
    }

    /**
     * Override this method if you want to provide a custom authentication context for initiating authentication requests.
     * <p/>
     * The default implementation creates a context based on configuration defaults (see {@link LinkIDSDKConfig}), which doesn't force
     ...
     */
    @NotNull
    protected static LinkIDAuthenticationContext newContext(HttpServletRequest request) {

        return new LinkIDAuthenticationContext( null, null, null, false, null, request.getRequestURL().toString() );
    }
