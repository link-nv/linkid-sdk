...
import net.link.safeonline.beid.webapp.AuthenticationPage.Goal;
...
public class AuthenticationPageTest {
    ...
    private FormTester prepareAuthentication(Goal goal)
        ...
        switch (goal) {
            case AUTHENTICATE:
                wicket.startPage(new UrlPageSource(BeIdApplication.AUTHENTICATION_MOUNTPOINT));
            break;

            case ENABLE_DEVICE:
                wicket.startPage(new UrlPageSource(BeIdApplication.ENABLE_MOUNTPOINT));
            break;
    ...
    public void testAuthenticate()
        ...
        mockBeIdDeviceService.checkMobile(TEST_MOBILE);
        expect(mockBeIdDeviceService.requestOTP(TEST_MOBILE)).andStubReturn(TEST_CHALLENGE);
        expect(mockBeIdDeviceService.authenticate(TEST_MOBILE, TEST_CHALLENGE, TEST_OTP)).andStubReturn(TEST_USERID);
    ...
    public void testAuthenticateFailed()
        ...
        mockBeIdDeviceService.checkMobile(TEST_MOBILE);
        expect(mockBeIdDeviceService.requestOTP(TEST_MOBILE)).andStubReturn(TEST_CHALLENGE);
        expect(mockBeIdDeviceService.authenticate(TEST_MOBILE, TEST_CHALLENGE, TEST_OTP)).andStubReturn(null);
        replay(mockBeIdDeviceService, mockSamlAuthorityService, mockHelpdeskManager);
    ...
    public void testEnable()
        ...
        // Describe Expected Scenario.
        expect(mockBeIdDeviceService.requestOTP(TEST_MOBILE)).andStubReturn(TEST_CHALLENGE);
        expect(mockBeIdDeviceService.authenticateEncap(TEST_CHALLENGE, TEST_OTP)).andReturn(true);
        mockBeIdDeviceService.enable(TEST_USERID, TEST_MOBILE);
        replay(mockBeIdDeviceService, mockSamlAuthorityService, mockHelpdeskManager);
    ...
    public void testEnableAuthenticateFailed()
        ...
        // Describe Expected Scenario.
        expect(mockBeIdDeviceService.requestOTP(TEST_MOBILE)).andStubReturn(TEST_CHALLENGE);
        expect(mockBeIdDeviceService.authenticateEncap(TEST_CHALLENGE, TEST_OTP)).andReturn(false);
        replay(mockBeIdDeviceService, mockSamlAuthorityService, mockHelpdeskManager);
