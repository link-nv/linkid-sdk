 ...
 * 
 * <p>
 * The chart generated by this module is a stacked area chart stacking the
 * request time of each driver used in the scenario below the agent overhead
 * time.<br>
 * <br>
 * Request time is defined as the time spent on the OLAS server during the
 * {@link ProfileFilter}'s
 * {@link FilterChain#doFilter(javax.servlet.ServletRequest, javax.servlet.ServletResponse)}
 * call. This basically involves all capturable time spent on the OLAS
 * Application Server for each web request made to it.<br>
 * <br>
 * Agent Overhead is defined as the time spent executing a scenario as recorded
 * by the agent minus the cumulative driver request times. This basically
 * involves remote request latency and latency introduced by agent operations
 * such as gathering and persisting of statistical data.
 * </p>
 * 
 * <p>
 * <i>Feb 22, 2008</i>
...
public class ScenarioDurationsChart extends AbstractChart {
	...
	private XYSeries getDriverSet(DriverProfileEntity profile) {
		...
		if (driverSet == null)
			this.driverSets.put(driverName, driverSet = new XYSeries(
					driverName, true, false));
