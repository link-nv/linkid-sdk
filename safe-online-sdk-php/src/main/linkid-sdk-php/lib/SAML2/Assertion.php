...
class SAML2_Assertion implements SAML2_SignedElement {
	...
	private function parseAttributes(DOMElement $xml) {
		...
		foreach ($attributes as $attribute) {
            ...
            /*
             * LinkID Attribute parsing, only need to handle compounds special, rest is ok...
             *
             * Formats:
             *  - single valued:
             *          /Attribute/AttributeValue/foo
             *  - multi valued:
             *          /Attribute/AttributeValue/foo
             *          /Attribute/AttributeValue/bar
             *  - compound:
             *          /Attribute@name="compoundname"/
             *                                          AttributeValue/
             *                                                        Attribute/AttributeValue/foo
             *                                                        Attribute/AttributeValue/bar
             *                                          AttributeValue/
            ...
            if (count($compoundMembers) > 0) {
                ...
                // parse...
                $members = array();
                foreach($compoundMembers as $compoundMember) {

                    $memberName = $compoundMember->getAttribute('Name');
                    if ($compoundMember->hasAttribute('NameFormat')) {
                        $memberNameFormat = $compoundMember->getAttribute('NameFormat');
                    } else {
                        $memberNameFormat = SAML2_Const::NAMEFORMAT_UNSPECIFIED;
                    }

                    if (!array_key_exists($memberName, $members)) {
                        $members[$memberName] = array();
                    }

                    $memberValues = SAML2_Utils::xpQuery($compoundMember, './saml_assertion:AttributeValue');
                    foreach ($memberValues as $memberValue) {
                        $members[$memberName][] = trim($memberValue->textContent);
                    }
                }

                $this->attributes[$name][] = $members;
