 ...
 * 
 * <p>
 * Applications can only retrieve attributes for which the user did confirm the corresponding application identity. Application also cannot
 * retrieve attributes that are marked as data-mining attribute types.
 * </p>
 * 
 * @author fcorneli
 * 
...
public interface AttributeService extends SafeOnlineService {
     ...
     * application and the attributes returned are those that have been confirmed by the user.
     * 
     * <p>
     * It is possible that a user already confirmed an attribute usage over an attribute that he still needs to define. In this case the
     * resulting map will not contain an entry for the missing attribute.
     * </p>
     * 
     * <p>
     * The type of map values depends on the actual datatype used by the corresponding attribute type of the attribute. In case of a
     * multivalued attribute the map value will be an array.
     * </p>
     * 
     * @param subjectLogin
     * @throws PermissionDeniedException
