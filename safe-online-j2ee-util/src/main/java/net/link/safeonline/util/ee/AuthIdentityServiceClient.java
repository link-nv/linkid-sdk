...
/**
 * Client for SafeOnline Authentication Identity Service JMX bean. This service
 * manages the key pair of the SafeOnline service itself. See also:
 * safe-online-service.
 * 
 * @author fcorneli
 * 
...
public class AuthIdentityServiceClient {
	...
	private static final Log LOG = LogFactory
			.getLog(AuthIdentityServiceClient.class);
	...
	public PrivateKey getPrivateKey() {
		...
		try {
			privateKey = (PrivateKey) this.server.invoke(
					this.authIdentityServiceName, "getPrivateKey", params,
					signature);
		} catch (Exception e) {
	...
	public PublicKey getPublicKey() {
		...
		try {
			publicKey = (PublicKey) this.server
					.invoke(this.authIdentityServiceName, "getPublicKey", params,
							signature);
		} catch (Exception e) {
			throw new RuntimeException("invoke error: " + e.getMessage(), e);
		}
	...
	public X509Certificate getCertificate() {
		...
		try {
			certificate = (X509Certificate) this.server.invoke(
					this.authIdentityServiceName, "getCertificate", params,
					signature);
		} catch (Exception e) {
			throw new RuntimeException("invoke error: " + e.getMessage(), e);
		}
