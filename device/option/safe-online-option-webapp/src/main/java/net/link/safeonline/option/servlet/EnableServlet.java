 ...
 * 
 */
public class EnableServlet extends AbstractNodeInjectionServlet {
    ...
    private void handleLanding(HttpServletRequest request, HttpServletResponse response)
        ...
        LOG.debug("enable option device: " + attributeId + " for user " + userId);

        ProtocolContext protocolContext = ProtocolContext.getProtocolContext(request.getSession());
        protocolContext.setSuccess(false);

        try {
            optionDeviceService.enable(userId, attributeId);
