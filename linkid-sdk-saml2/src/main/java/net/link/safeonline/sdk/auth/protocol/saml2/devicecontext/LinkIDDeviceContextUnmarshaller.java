...
public class LinkIDDeviceContextUnmarshaller extends AttributeStatementUnmarshaller {

    @Override
    protected void processChildElement(XMLObject parentObject, XMLObject childObject)
            throws UnmarshallingException {

        LinkIDDeviceContext linkIDDeviceContext = (LinkIDDeviceContext) parentObject;

        if (childObject instanceof Attribute) {
            linkIDDeviceContext.getAttributes().add( (Attribute) childObject );
        } else if (childObject instanceof EncryptedAttribute) {
            linkIDDeviceContext.getEncryptedAttributes().add( (EncryptedAttribute) childObject );
        } else {
