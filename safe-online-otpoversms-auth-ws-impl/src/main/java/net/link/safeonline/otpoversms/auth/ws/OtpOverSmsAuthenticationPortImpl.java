 ...
 * 
 * 
 * Do NOT use {@link Injection} as this is a {@link Stateful} web service and the statefulness is achieved by JAX-WS
 * using the same {@link InstanceResolver} as is used by the {@link Injection}.
 * 
 * @author wvdhaute
 * 
 */
...
public class OtpOverSmsAuthenticationPortImpl implements DeviceAuthenticationPort {
    ...
    class TimeoutCallback implements StatefulWebServiceManager.Callback<DeviceAuthenticationPort> {
         ...
         * {@inheritDoc}
         */
        public void onTimeout(DeviceAuthenticationPort timedOutObject,
                              StatefulWebServiceManager<DeviceAuthenticationPort> serviceManager) {
    ...
    private WSAuthenticationResponseType sendOtp(WSAuthenticationRequestType request) {
        ...
        SamlAuthorityService samlAuthorityService = EjbUtils.getEJB( SamlAuthorityService.JNDI_BINDING,
                                                                     SamlAuthorityService.class );
        String issuerName = samlAuthorityService.getIssuerName();
        WSAuthenticationResponseType response = DeviceAuthenticationPortUtil.generateResponse( request.getID(),
                                                                                               issuerName,
        ...
        if (null == mobile) {
            DeviceAuthenticationPortUtil.setStatus( response, WSAuthenticationErrorCode.INSUFFICIENT_CREDENTIALS,
                                                    "\"" + OtpOverSmsConstants.OTPOVERSMS_WS_AUTH_MOBILE_ATTRIBUTE
                                                            + "\" is not specified" );
        ...
        try {
            ...
            DeviceAuthenticationPortUtil.setStatus( response, WSAuthenticationErrorCode.SUBJECT_NOT_FOUND,
                                                    e.getMessage() );
            manager.unexport( this );
            return response;
        } catch (DeviceRegistrationNotFoundException e) {
            LOG.error( "device not registered: " + e.getMessage(), e );
            DeviceAuthenticationPortUtil.setStatus( response, WSAuthenticationErrorCode.DEVICE_NOT_FOUND,
                                                    e.getMessage() );
            manager.unexport( this );
            return response;
    ...
    private WSAuthenticationResponseType verifyOtpAndAuthenticate(WSAuthenticationRequestType request) {
        ...
        if (null == pin || null == otp) {
            DeviceAuthenticationPortUtil.setStatus( response, WSAuthenticationErrorCode.INSUFFICIENT_CREDENTIALS,
                                                    "\"" + OtpOverSmsConstants.OTPOVERSMS_WS_AUTH_OTP_ATTRIBUTE
                                                            + "\" or \""
                                                            + OtpOverSmsConstants.OTPOVERSMS_WS_AUTH_PIN_ATTRIBUTE
                                                            + "\" is not specified" );
            return response;
        }
