...
public class SafeOnlineNodeLoginModule implements LoginModule {
    ...
    public boolean commit() throws LoginException {
        ...
        rolesGroup.addMember(new SimplePrincipal(SafeOnlineNodeRoles.NODE_ROLE));
    ...
    public boolean login() throws LoginException {
        ...
        PasswordCallback passwordCallback = new PasswordCallback("X509 node certificate in Hex", false);
        NameCallback nameCallback = new NameCallback("node name");
        ...
        // authenticate
        NodeAuthenticationService nodeAuthenticationService = getNodeAuthenticationService();
        String nodeName;
        try {
            nodeName = nodeAuthenticationService.authenticate(certificate);
        } catch (NodeNotFoundException e) {
            throw new FailedLoginException("certificate is not a node certificate");
        }

        String expectedNodeName = nameCallback.getName();
        if (false == nodeName.equals(expectedNodeName))
            throw new FailedLoginException("node name not correct");

        this.authenticatedPrincipal = new SimplePrincipal(nodeName);
        LOG.debug("login: " + nodeName);
        LOG.debug("login subject: " + this.subject);

        return true;
    }

    private NodeAuthenticationService getNodeAuthenticationService() throws LoginException {

        try {
            NodeAuthenticationService nodeAuthenticationService = EjbUtils.getEJB(this.authenticationServiceJndiName,
                    NodeAuthenticationService.class);
            return nodeAuthenticationService;
