...
public class GetBeIdAuthenticationPortImpl implements GetDeviceAuthenticationPort {

    private static final Log LOG = LogFactory.getLog(GetBeIdAuthenticationPortImpl.class);


    @WebMethod
    public synchronized AuthenticationGetInstanceResponseType getInstance(AuthenticationGetInstanceRequestType request) {

        LOG.debug("return instance of stateful authentication webservice for BeId device");
        AuthenticationGetInstanceResponseType response = new AuthenticationGetInstanceResponseType();
        response.setEndpoint(BeIdAuthenticationPortImpl.manager.export(new BeIdAuthenticationPortImpl()));
        return response;
