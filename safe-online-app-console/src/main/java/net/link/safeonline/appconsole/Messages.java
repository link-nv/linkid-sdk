...
public enum Messages {
	...
	TITLE("title"), IDENTITY("identity"), LOCATION("location"), FILE("file"), SERVICES(
			...
			"certificate"), PRIVATE_KEY("private_key"), KEYSTORE("keystore"), KEYSTORE_TYPE(
			"keystore_type"), KEYSTORE_PW("keystore_pw"), KEYENTRY_PW(
			"keyentry_pw"), ERROR_MISSING_FIELDS("error_missing_fields"), ERROR_SELECT_KEYSTORE(
			...
			"user"), AUTH_USER("auth_user"), DEBUG("debug"), REFRESH("refresh"), CAPTURE(
			"capture"), VIEW_INBOUND_SOAP("view_inbound_soap"), VIEW_OUTBOUND_SOAP(
			"view_outbound_soap"), UTILS("utils"), DONE("done"), CONFIRM_ID(
			"confirm_identity"), CERT_ISSUER_DN("cert_issuer_dn"), CERT_SUBJECT_DN(
