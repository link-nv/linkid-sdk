...
/**
 * Test SOAP JAX-WS Handler to verify the different XML Signature of the STS
 * request.
 * 
 * @author fcorneli
 * 
 */
public class SignatureVerificationTestHandler implements
		SOAPHandler<SOAPMessageContext> {

	private static final Log LOG = LogFactory
			.getLog(SignatureVerificationTestHandler.class);
	...
	private void verifySamlpSignature(Document document,
			X509Certificate certificate) throws TransformerException,
			XMLSignatureException, XMLSecurityException {
		Element nsElement = createNsElement(document);
		Element samlpSignature = (Element) XPathAPI.selectSingleNode(document,
	...
	private void verifyWSSecuritySignature(Document document,
			X509Certificate certificate) throws TransformerException,
			XMLSignatureException, XMLSecurityException {
		Element nsElement = createNsElement(document);
